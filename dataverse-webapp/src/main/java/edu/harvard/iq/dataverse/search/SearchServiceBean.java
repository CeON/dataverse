...
//                    logger.info("adding <<<" + staticSearchField + ":" + friendlyName + ">>>");
     ...
     * @param numResultsPerPage
     * @param retrieveEntities     - look up dvobject entities with .find() (potentially expensive!)
     * @param idsToKeep - List of Entity Ids we keep from solr query, all others are removed
     * @return
     * @throws SearchException
     */
    public SolrQueryResponse search(DataverseRequest dataverseRequest, List<Dataverse> dataverses, String query,
                                    List<String> filterQueries, String sortField, String sortOrder, int paginationStart,
                                    boolean onlyDatatRelatedToMe, int numResultsPerPage, boolean retrieveEntities, List<Long> idsToKeep)
            throws SearchException {
        ...
        SolrDocumentList docs = queryResponse.getResults();
        // make sure elements order doesnt affect results
        Collections.sort(idsToKeep);
        SolrDocumentList tempdocs = new SolrDocumentList();
        for(SolrDocument doc : docs) {
            if(idsToKeep.contains(doc.get("entityId"))) {
                tempdocs.add(doc);
            }
        }
        if(tempdocs.size() == 0) {
            throw new IllegalArgumentException("There are no results for this page");
        }
        int endIndex = ((paginationStart + numResultsPerPage) >  tempdocs.size()) ? tempdocs.size() : (paginationStart + numResultsPerPage);
        docs.clear();
        for(int i=0; i <= tempdocs.size(); i++) {
            if(i >= paginationStart && i < endIndex) {
                docs.add(tempdocs.get(i));
            }
        }
...
//                solrSearchResult.setApiUrl(baseUrl + "/api/meta/datafile/" + entityid);
        ...
        boolean deaccessionedAvailable = false;
        boolean hideMetadataSourceFacet = true;
        List<String> facetsCounters = updateFacetFieldsCounts(tempdocs);
        for (FacetField facetField : queryResponse.getFacetFields()) {
            FacetCategory facetCategory = new FacetCategory();
            List<FacetLabel> facetLabelList = new ArrayList<>();
            int numMetadataSources = 0;
            for (FacetField.Count facetFieldCount : facetField.getValues()) {
                ...
                if(facetsCounters.contains(facetFieldCount.getName())) {
                    facetFieldCount.setCount(Collections.frequency(facetsCounters, facetFieldCount.getName()));
                } else {
                    facetFieldCount.setCount(0);
                }
...
//                    logger.info("adding <<<" + staticSearchField + ":" + friendlyName + ">>>");
        ...
        solrQueryResponse.setFacetCategoryList(facetCategoryList);
        solrQueryResponse.setTypeFacetCategories(typeFacetCategories);
        solrQueryResponse.setNumResultsFound((long)tempdocs.size());
        solrQueryResponse.setResultsStart(queryResponse.getResults().getStart());
        solrQueryResponse.setDatasetfieldFriendlyNamesBySolrField(datasetfieldFriendlyNamesBySolrField);
        solrQueryResponse.setStaticSolrFieldFriendlyNamesBySolrField(staticSolrFieldFriendlyNamesBySolrField);
        String[] filterQueriesArray = solrQuery.getFilterQueries();
